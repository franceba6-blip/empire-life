...
export const monthlyExpenses = (state: GameState) => state.transactions.filter(t => t.gameDate.year === state.date.year && t.gameDate.month === state.date.month && ['LOAN_PAYMENT', 'FAMILY_LOAN_REPAYMENT', 'BUSINESS_PURCHASE', 'BUSINESS_EXPENSE'].includes(t.type)).reduce((s, t) => s + t.amount, 0);

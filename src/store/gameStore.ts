...
interface Store {
  ...
  work: (jobId: string) => { reward: number; hours: number }; advanceDay: () => void; reset: () => Promise<void>;
}
...
export const useGameStore = create<Store>((set, get) => ({
  ...
  work: jobId => { const job = JOBS.find(j => j.id === jobId)!; const reward = Math.round(job.minReward + Math.random() * (job.maxReward - job.minReward)); get().update(game => { let next = transact(game, 'JOB_INCOME', reward, job.name); next = advanceGameTime(next, job.timeHours); return { ...next, stats: { ...next.stats, jobsCompleted: next.stats.jobsCompleted + 1 } }; }); return { reward, hours: job.timeHours }; },

...
import { ROLE_LABELS, ROLE_TASKS, STAFF_TASKS, TASK_LABELS } from "@/game/employees/config";
...
export default function Management() {
  ...
  if (!game) return null;
  const business = game.businesses[0], employees = game.staff.employees.filter((employee) => employee.businessId === business.id);
  const managers = employees.filter((employee) => ["BUSINESS_MANAGER", "GENERAL_MANAGER"].includes(employee.role));
  const choose = (task: StaffTask) => {
    ...
    Alert.alert(TASK_LABELS[task], "Who should own this task?", [
      ...
      ...compatible.slice(0, 4).map((employee) => ({ text: `${employee.firstName} · ${employee.performance}%`, onPress: () => { try { update((state) => assignTask(state, business.id, task, ["TEAM_LEADER", "BUSINESS_MANAGER", "GENERAL_MANAGER"].includes(employee.role) ? "MANAGER" : "EMPLOYEE", employee.id)); } catch (error) { Alert.alert("Delegation failed", (error as Error).message); } } })),
      { text: "Cancel", style: "cancel" as const },
    ]);
  };
  const full = () => {
    if (!managers.length) return Alert.alert("Manager required", "Hire a Business Manager or General Manager first.");
  ...
  return <ScrollView style={s.root} contentContainerStyle={s.content}>
    ...
    <SectionTitle>Task delegation</SectionTitle>{STAFF_TASKS.map((task) => { const assignment = business.management.delegations.find((item) => item.task === task)!; return <Card key={task} style={s.task}><View style={s.row}><View style={{ flex: 1 }}><Text style={s.taskTitle}>{TASK_LABELS[task]}</Text><Text style={s.copy}>{assignment.mode} · {employeeName(assignment.employeeId)}</Text></View><Text style={s.mode}>{assignment.mode}</Text></View><PrimaryButton title="CHANGE ASSIGNMENT" tone="dark" onPress={() => choose(task)} /></Card>; })}

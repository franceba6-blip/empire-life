...
export default function Staff() {
  ...
  if (!game) return null;
  const selectedBusiness = game.businesses.find((item) => item.id === businessId) ?? game.businesses[0];
  const employees = game.staff.employees.filter((employee) => employee.businessId === selectedBusiness.id);
  ...
  return <ScrollView style={s.root} contentContainerStyle={s.content}>
    <PrimaryButton title="‹ BUSINESS" tone="dark" onPress={() => router.replace(selectedBusiness.type === "CAR_DEALERSHIP" ? "/dealership" as Href : "/(tabs)/business")} />
    <Text style={s.kicker}>{selectedBusiness.name.toUpperCase()} · STAFF</Text><Text style={s.title}>Build the team.</Text>
